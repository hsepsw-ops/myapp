...
<body>
    <div class="shell">
        ...
        <div class="section">
            <div class="field"><strong>Nama</strong><span>{{ $report['nama'] }}</span></div>
            <div class="field"><strong>NIK</strong><span>{{ $report['nik'] }}</span></div>
            <div class="field"><strong>Grup</strong><span>{{ $report['grup'] }}</span></div>
            <div class="field"><strong>Tanggal</strong><span>{{ $report['tanggal'] }}</span></div>
        </div>
        <div class="section">
            <div class="field"><strong>Yang Dikawal</strong><span>{{ $report['yang_dikawal'] }}</span></div>
            <div class="field"><strong>Rute Pengawalan</strong><span>{{ $report['rute_pengawalan'] }}</span></div>
            <div class="field"><strong>Evidence</strong><span>{{ $report['evidence'] === '-' ? 'Tidak ada file' : basename($report['evidence']) }}</span></div>
        </div>
        @if($report['evidence'] !== '-' && file_exists(public_path($report['evidence'])))
            <div class="section">
                <h2>Preview Evidence</h2>
                <div class="evidence-image-wrapper">
                    <img class="evidence-image" src="/{{ $report['evidence'] }}" alt="Evidence Pengawalan">
                </div>
            </div>
        @endif
        <div class="section">
            <div class="field"><strong>Waktu Dibuat</strong><span>{{ $report['created_at'] }}</span></div>
        </div>
